...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-200 backdrop-blur-sm sticky top-0 bg-white/80 z-50 shadow-sm' >
      ...
      <ul className='hidden sm:flex items-start gap-5 font-medium text-xs lg:text-sm lg:gap-8'>
      ...
      <div className='flex items-center gap-4'>
        <button 
          ...
          className='hidden sm:block py-2 px-3 lg:px-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-full font-medium hover:shadow-lg hover:scale-105 transition-all duration-300 text-xs lg:text-sm'
        >
          Admin
        </button>
        {
          user ?
            ...
            : 
            <>
              <button onClick={() => navigate('/login')} className='hidden sm:block py-2 px-4 lg:py-3 lg:px-8 bg-gradient-to-r from-[#5f6FFF] to-[#7a88ff] text-white rounded-full font-medium cursor-pointer hover:shadow-lg hover:scale-105 transition-all duration-300 text-xs lg:text-sm'>Login</button>
              <img onClick={() => setShowMenu(true)} src={assets.menu_icon} className='w-6 cursor-pointer sm:hidden hover:scale-110 transition-transform duration-300' alt="" />
            </>
        }
